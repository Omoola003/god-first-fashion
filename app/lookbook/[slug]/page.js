...
export async function generateStaticParams() {
  ...
  return lookbooks.map((s) => ({ slug: s.slug }));
}
...
export default async function LookbookDetailPage({ params }) {
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white selection:bg-white/20">
      {/* HERO SECTION - Only render if coverImage has an asset */}
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
        {lookbook.coverImage?.asset ? (
          ...
          />
           ...
           <div className="absolute inset-0 bg-neutral-800" /> // Fallback background
        )}
        <div className="absolute inset-0 bg-black/40" />
        <div className="relative z-10 text-center px-4">
          <span className="uppercase tracking-[0.4em] text-sm text-neutral-300 block mb-6">Editorial Series</span>
          <h1 className="font-serif text-6xl md:text-8xl mb-4 tracking-tight">{lookbook.title}</h1>
          <p className="text-neutral-300 max-w-xl mx-auto font-light leading-relaxed">{lookbook.description}</p>
        </div>
      </section>

      {/* EDITORIAL STORY SECTIONS */}
      {lookbook.images && lookbook.images.length > 0 && lookbook.images[0]?.asset && (
        <section className="max-w-6xl mx-auto px-4 py-32 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="order-2 md:order-1">
            ...
            <p className="text-neutral-400 font-light leading-relaxed text-lg mb-8">
              Authority is no longer loud. It is composed, deliberate, and deeply rooted in heritage.
            </p>
            <div className="h-px w-24 bg-white/20" />
          </div>
          <div className="relative aspect-[3/4] order-1 md:order-2 shadow-2xl bg-neutral-800">
            <Image
      ...
      <section className="max-w-7xl mx-auto px-4 pb-40">
        <h3 className="text-center font-serif text-3xl mb-16 uppercase tracking-widest">Visual Impressions</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {lookbook.images?.map((img, index) => {
            // CRITICAL FIX: Skip the image if the asset is missing
            if (!img?.asset) return null;

            return (
              <div key={img._key || index} className="relative aspect-[3/4] overflow-hidden group bg-neutral-800 shadow-lg">
                <Image
